-- =====================================================
-- ABAN REMIT - PRODUCTION SCHEMA DEPLOYMENT
-- =====================================================
-- For use with phpPgAdmin (no psql meta-commands)
-- Database: fkmqtves_aban_remit
-- PostgreSQL 10.23+
-- =====================================================

-- SECTION 1: EXTENSIONS AND SETTINGS
-- Note: Extensions may not be available on shared hosting
-- We'll use alternative UUID generation if extensions fail
DO $$
BEGIN
  CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'uuid-ossp extension not available, will use alternative UUID generation';
END $$;
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'pgcrypto extension not available, will use alternative functions';
END $$;

SET timezone = 'UTC';

-- Alternative UUID generation function if uuid-ossp is not available
CREATE OR REPLACE FUNCTION public.generate_uuid_v4()
RETURNS UUID AS $$
DECLARE
  value TEXT;
BEGIN
  value = md5(random()::TEXT || clock_timestamp()::TEXT);
  RETURN (
    substr(value, 1, 8) || '-' ||
    substr(value, 9, 4) || '-4' ||
    substr(value, 13, 3) || '-' ||
    ('{8,9,a,b}'::TEXT[])[floor(random() * 4 + 1)::INT] ||
    substr(value, 17, 3) || '-' ||
    substr(value, 21, 12)
  )::UUID;
END;
...
-- Use gen_random_uuid if available, otherwise use our custom function
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN public.generate_uuid_v4();
  END;
END;
$$ LANGUAGE plpgsql VOLATILE;
...
-- SECTION 10: RECONCILIATION SCHEMA
...
-- DEPLOYMENT COMPLETE
-- Run verification queries below to confirm successful deployment

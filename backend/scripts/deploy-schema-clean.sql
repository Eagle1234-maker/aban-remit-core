-- =====================================================
-- ABAN REMIT - COMPLETE SCHEMA DEPLOYMENT (Clean Version)
-- =====================================================
-- This version has no psql meta-commands (\set, \echo, etc.)
...
-- =====================================================
-- SECTION 1: EXTENSIONS AND SETTINGS (OPTIONAL)
-- =====================================================

-- Try to create extensions if available (will skip if not installed)
-- These are optional - we use built-in functions as fallback
DO $$ 
BEGIN
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'pgcrypto extension not available, using built-in functions instead';
END $$;

SET timezone = 'UTC';

SELECT 'Extensions checked (using built-in functions if extensions unavailable)' as status;

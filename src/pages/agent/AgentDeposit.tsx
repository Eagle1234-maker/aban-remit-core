...
const AgentDeposit = () => {
  ...
  const handleDeposit = async () => {
    ...
    toast.success(`KES ${Number(amount).toLocaleString()} deposited to ${wallet}`);
    setWallet(''); setAmount('');
    setLoading(false);
  };
    ...
    <div className="max-w-lg mx-auto space-y-6 animate-fade-in">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <Button className="w-full" disabled={!wallet || !amount || loading} onClick={handleDeposit}>
            {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null} Deposit
          </Button>
        </CardContent>
      </Card>
    </div>
